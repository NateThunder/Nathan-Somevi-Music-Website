...
export async function listTable<T>(table: TableName) {
  const rows = await fetchRows<T>(table, { orderBy: "order_index" });
  return NextResponse.json({ data: rows });
}

export async function createTableRow<T>(table: TableName, payload: Record<string, unknown>) {
...
export async function reorderTable(table: TableName, ids: string[]) {
  await Promise.all(
